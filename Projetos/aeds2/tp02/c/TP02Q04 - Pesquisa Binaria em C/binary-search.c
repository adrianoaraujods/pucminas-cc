...
void sortShows(Show *shows[], int left, int right) {
  ...
  while (i <= j) {
    ...
    while (strcmp(shows[j]->title, pivot) > 0) j--;
  ...
  if (left < j)  sortShows(shows, left, j);
  if (i < right)  sortShows(shows, i, right);
}
...
bool binarySearch(Show **shows, int count, char *title) {
  ...
  while (left <= right) {
    ...
    if (strcmp(shows[pivot]->title, title) < 0) {
      comparisons++; // benchmark
      left = pivot + 1;
    } else if (strcmp(shows[pivot]->title, title) > 0) {
      comparisons++; // benchmark
      right = pivot - 1;
    } else {
      found = true;
...
int main() {
  ...
  sortShows(shows, 0, count - 1);

  while (strcmp("FIM", line = readLine()) != 0) {
    printf("%s\n", binarySearch(shows, count, line) ? "SIM" : "NAO");
