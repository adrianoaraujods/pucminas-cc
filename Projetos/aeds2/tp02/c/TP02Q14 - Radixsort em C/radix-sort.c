...
int getMax(Show* shows[], int n) {
  int max = shows[0]->release_year;

  for (int i = 1; i < n; i++) {
    comparisons++;

    if (max < shows[i]->release_year) {
...
void RadixCountingSort(Show* shows[], int n, int exp) {
  int count[10];
  Show* output[n];
  ...
  for (int i = 0; i < n; i++) {
    ...
    count[(shows[i]->release_year / exp) % 10]++;
  }
  ...
  for (int i = n - 1; i >= 0; i--) {
    ...
    output[count[(shows[i]->release_year / exp) % 10] - 1] = shows[i];
    count[(shows[i]->release_year / exp) % 10]--;
  }

  for (int i = 0; i < n; i++) shows[i] = output[i];
}

void sort(Show* shows[], int n) {
  int max = getMax(shows, n);

  for (int exp = 1; max / exp > 0; exp *= 10) {
    RadixCountingSort(shows, n, exp);
  }
}
...
void untie(Show* shows[], int n) {
  for (int i = 1; i < n; i++) {
    ...
    while ((j >= 0) &&
      (shows[j]->release_year == temp->release_year) &&
...
int main() {
  ...
  FILE* file = fopen("matricula_radixsort.txt", "w");
