...
class ShowList {
    ...
    public void sort() { // by date_added
        comparisons = 0;
        quicksort(first.right, last);
    }

    public void quicksort(ShowNode start, ShowNode end) {
        if (start == null || end == null || start == end || start == end.right) {
            return;
        }

        ShowNode pivotNode = partition(start, end);

        if (pivotNode != null && pivotNode.left != null) {
            quicksort(start, pivotNode.left);
        }
        if (pivotNode != null && pivotNode.right != null) {
            quicksort(pivotNode.right, end);
        }
    }

    private ShowNode partition(ShowNode start, ShowNode end) {
        ...
        for (ShowNode j = start; j != end; j = j.right) {
            ...
            if (j.element.date_added.compareTo(pivot.date_added) <= 0) {
    ...
    private void swap(ShowNode a, ShowNode b) {
        Show temp = a.element;
        a.element = b.element;
        b.element = temp;
    }

    public void untie() { // by title
        for (ShowNode i = first.right; i != last; i = i.right) {
            Show element = i.element;

            ShowNode j = i.left;
            while (j != first
                    && j.element.date_added.compareTo(element.date_added) == 0
                    && j.element.getTitle().compareToIgnoreCase(element.getTitle()) > 0) {
                comparisons++;
            ...
            j.right.element = element;
        }
    }

    public void print() {
        for (ShowNode node = first.right; node != null; node = node.right) {
            node.element.print();
        }
    }
}

public class ShowDoublyLinkedListQuicksort {
    ...
    public static void main(String[] args) {
        ...
        try (FileWriter writer = new FileWriter("matricula_quicksort.txt")) {
            writer.write("000000" + '\t' + (end - start) + '\t' + list.comparisons);
        } catch (IOException e) {
            //
        }
    }
