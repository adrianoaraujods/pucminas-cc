...
typedef struct {
  Show* array[QUEUE_LENGTH];
  int bottom;
  int head;
} ShowCircularQueue;

bool isEmpty(ShowCircularQueue* queue) {
  return queue->head == queue->bottom;
}

bool isFull(ShowCircularQueue* queue) {
  return (queue->head + 1) % QUEUE_LENGTH == queue->bottom;
}

int getLength(ShowCircularQueue* queue) {
  return (queue->head - queue->bottom + QUEUE_LENGTH) % QUEUE_LENGTH;
}

int getQueueAverage(ShowCircularQueue* queue) {
  int length = getLength(queue);
  if (length == 0) return 0;

  int sum = 0;

  for (int i = queue->bottom; i != queue->head; i = (i + 1) % QUEUE_LENGTH) {
    sum += queue->array[i]->release_year;
  }

  return sum / length;
}

Show* dequeue(ShowCircularQueue* queue) {
  if (isEmpty(queue)) return NULL;

  Show* show = queue->array[queue->bottom];
  queue->bottom = (queue->bottom + 1) % QUEUE_LENGTH;

  return show;
}

void enqueue(ShowCircularQueue* queue, Show* show) {
  if (isFull(queue)) dequeue(queue);

  queue->array[queue->head] = show;
  queue->head = (queue->head + 1) % QUEUE_LENGTH;

  printf("[Media] %d\n", getQueueAverage(queue));
}

void printShowCircularQueue(ShowCircularQueue* queue) {
  for (int i = queue->bottom; i != queue->head; i = (i + 1) % QUEUE_LENGTH) {
    printf("[%d] ", i - 1);
    printShow(queue->array[i]);
  }
}

int main() {
  ShowCircularQueue queue;
  queue.bottom = 0;
  queue.head = 0;
  char* line;
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (line[0] == 'I') {
      int index = atoi(substring(line, 3, strlen(line) - 1));
      char* input = fileLine(index);
      show = parseShow(input);
      free(input);
