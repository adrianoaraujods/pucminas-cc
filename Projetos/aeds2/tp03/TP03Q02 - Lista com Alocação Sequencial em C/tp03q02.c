...
typedef struct {
  Show* array[MAX_LIST_LENGTH];
  int length;
} ShowList;

void insert(ShowList* list, Show* show, int index) {
  if (
    index < 0 || index > list->length || list->length >= MAX_LIST_LENGTH - 1
    ) return;
...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    line = readLine();
    char operation[] = { line[0], line[1], '\0' };

    if (operation[0] == 'I') {
      int i = 1;
      while (line[strlen(line) - i] != 's') i++;

      int index = atoi(substring(line, strlen(line) - i + 1, strlen(line) - 1));
      ...
      if (input) {
        ...
        if (operation[1] == '*') {
          ...
          while (line[strlen(line) - j] != ' ') j++;

          index = atoi(substring(line, strlen(line) - j - 2, strlen(line) - i - 2));

          insert(&list, show, index);
        } else if (operation[1] == 'I') {
          insertStart(&list, show);

        } else if (operation[1] == 'F') {
          append(&list, show);
        } else {
          printf("\n\aOperação não suportada.");
        }
      } else {
        printf("\n\aOperação não suportada.");
      }
      ...
      if (operation[1] == '*') {
        int i = 1;
        while (line[strlen(line) - i] != ' ') i++;

        int index = atoi(substring(line, strlen(line) - i + 1, strlen(line) - 1));

        show = removeIndex(&list, index);

      } else if (operation[1] == 'I') {
        show = removeStart(&list);
