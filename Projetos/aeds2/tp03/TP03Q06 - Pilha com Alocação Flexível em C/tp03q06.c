...
void add(ShowStack* stack, Show* show) {
  ShowNode* newNode = malloc(sizeof(ShowNode));
  newNode->element = show;
  newNode->prev = NULL;

  if (stack->length > 0) newNode->prev = stack->head;
...
Show* retrive(ShowStack* stack) {
  if (stack->length <= 0) return NULL;

  ShowNode* removedNode = stack->head;
  Show* show = removedNode->element;
...
void printShowStack(ShowStack* stack) {
  ShowNode* node = stack->head;

  for (int i = stack->length - 1; i >= 0; i--, node = node->prev) {
    printf("[%d] ", i);
