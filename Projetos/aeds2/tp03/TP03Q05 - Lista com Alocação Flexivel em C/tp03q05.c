...
void insertIndex(ShowList* list, Show* show, int index) {
  ...
  if (list->length == 0) { // insert first node
    ...
    for (int i = 0; i < index - 1; i++) node = node->right;
...
Show* removeIndex(ShowList* list, int index) {
  ...
  if (list->length == 0) { // remove only node
    ...
    ShowNode* removedNode = node->right;
    node->right = removedNode->right;

    show = removedNode->element;
    free(removedNode);

    if (index == list->length - 1) list->end = node;
  }
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (operation[0] == 'I') {
      ...
      if (input) {
        ...
        printf("\n\aErro ao ler linha da planilha.");
      }
