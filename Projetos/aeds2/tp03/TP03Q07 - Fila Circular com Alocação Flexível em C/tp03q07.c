...
int getQueueAverage(ShowCircularQueue* queue) {
  ...
  int sum = 0;
  ShowNode* node = queue->bottom;

  for (int i = 0; i < queue->length; i++, node = node->right) {
    sum += node->element->release_year;
  }
...
Show* dequeue(ShowCircularQueue* queue) {
  ...
  ShowNode* prevBottom = queue->bottom;
  Show* show = prevBottom->element;

  if (queue->length == 1) {
    queue->bottom = queue->head = NULL;
  } else {
    queue->bottom = prevBottom->right;
    queue->head->right = queue->bottom;
  }
...
void enqueue(ShowCircularQueue* queue, Show* show) {
  ...
  newNode->right = queue->bottom;

  if (isEmpty(queue)) {
    queue->bottom = newNode;
  } else {
    queue->head->right = newNode;
  }
