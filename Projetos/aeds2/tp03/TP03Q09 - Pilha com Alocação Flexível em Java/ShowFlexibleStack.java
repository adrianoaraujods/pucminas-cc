...
class ShowStack {
    ...
    public void print() {
        ShowNode node = top;
        for (int i = length - 1; i >= 0; node = node.right, i--) {
            System.out.println("[" + i + "]" + " " + node.element.toString());
        }
    }
}

public class ShowFlexibleStack {

    static Scanner scanner = new Scanner(System.in);
    static ShowStack stack = new ShowStack();
    ...
    public static void main(String[] args) throws Exception {
        ...
        while ((line = scanner.nextLine()).compareTo("FIM") != 0) {
            ...
            stack.add(Show.read(File.readLine(index)));
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (operation) {
                case "I" -> {
                    int ShowIndex = Integer.parseInt(params[1].substring(1));
                    stack.add(Show.read(File.readLine(ShowIndex)));
                }
                case "R" -> {
                    System.out.println("(R) " + stack.remove().getTitle());
                }
                default ->
