...
public class SortingByCountingSort {
    ...
    static void sort() {
        ...
        for (int i = n - 1; i >= 0; i--) {
            comparisons++;
            ordenado[count[shows[i].getReleaseYear()] - 1] = shows[i];
            count[shows[i].getReleaseYear()]--;
        }

        System.arraycopy(ordenado, 0, shows, 0, n);
    }
    ...
    static void untie() {
        for (int i = 1; i < n; i++) {
            ...
            while ((j >= 0)
                    && (shows[j].getReleaseYear() == temp.getReleaseYear() && (shows[j].getTitle()).compareToIgnoreCase(temp.getTitle()
            ) > 0)) {
                comparisons++;
                shows[j + 1] = shows[j];
                j--;
